...
def test_split_sequence(tokenizer):
    ...
    split_long2 = tokenization.split_sequences(tokenizer, [long_text, long_text])
    ...
    half = int(len(split_long2) / 2)
    for split_text in split_long2[: half - 2]:
        assert len(tokenizer(split_text, add_special_tokens=False)['input_ids']) == tokenizer.max_len_single_sentence
    assert len(tokenizer(split_long2[half - 1], add_special_tokens=False)['input_ids']) <= tokenizer.max_len_single_sentence
    
    for split_text in split_long2[half: half - 2]:
        assert len(tokenizer(split_text, add_special_tokens=False)['input_ids']) == tokenizer.max_len_single_sentence
    assert len(tokenizer(split_long2[-1], add_special_tokens=False)['input_ids']) <= tokenizer.max_len_single_sentence

    assert ''.join(split_long2) == long_text*2